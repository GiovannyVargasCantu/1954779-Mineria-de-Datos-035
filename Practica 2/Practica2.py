...
def DataFrame_Clean():
    DataFrame = pd.read_csv('csv/DataSet.csv') #Se declara el dataframe, que lo lee de DataSet.csv, que sería el dataset que se descargo en la practica 1
    ...
    DataFrame.to_csv('csv/DataSet.csv', index=False) #Se actualiza el dataset, se pone el mismo nombre que el original para que lo reemplaze y solo se haya como actualizado, es decir
