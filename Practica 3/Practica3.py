...
def GenerarFuncionAgregacion(): #Se define el nombre de la función
    DataFrame = pd.read_csv('csv/DataSet.csv') #Lee el archivo DataSet.csv, recuerden qeu si somos de practica 2, este seria el archivo ya limpio y ordenado
    DataFrame = show_data_by_department(DataFrame) #Se aplica la función en el dataframe DataSet.csv
    DataFrame.to_csv('csv/Analyzed DataSet Departments.csv', index=False) #Aqui es para pasar el dataframe que acabamos de definir a un nuevo archivo csv llamado Analyzed DataSet Departments
